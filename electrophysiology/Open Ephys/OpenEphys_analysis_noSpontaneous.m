...
[fileName pathName]=uigetfile('*.mat','Select .mat file',...
    'D:\Data\optogenetics\Open Ephys\analized data\20211110');
...
% with signals
prompt = {'Which channels of interest?','Reference channels (0 for no reference)','bad channes'};
...
good_channels = data_file.Data;
if ephys.bad(1)>0 % if there is bad channels
    good_channels(ephys.bad,:) = nan;
end

ephys.median_noise = nanmedian(good_channels);
% ephys.median_noise = nanmean(good_channels);
% ephys.median_noise = 0;
...
% figure;
% for i =1:32
%     plot(data_file.Data(i,1:3.6668e+05));
%     hold on
% end
% 
% for channel_ref = 1:32
% %     ephys.refSpike_filtered(channel_ref,:) = bandpass(ref_volt(channel_ref,:),[250 8000],30000); % 250-8000 Hz for spikes
%     [b,a] = butter(4, [250 8000]/(30000/2));
%     test(channel_ref,:) = filtfilt(b,a,double(data_file.Data(channel_ref,1:3.6668e+05)));
% end
% 
% figure;
% for i =1:32
...
%     ephys.refSpike_filtered(channel_ref,:) = bandpass(ref_volt(channel_ref,:),[250 8000],30000); % 250-8000 Hz for spikes
    [b,a] = butter(2, [250 8000]/(30000/2));
    ephys.refSpike_filtered(channel_ref,:) = filter(b,a,double(ref_volt(channel_ref,:)));
end
...
%         [peak_noise,peak_noise_spike] = max(trace(locs(noise_spike)));
        if length(noise_spike)>1
            for noise = 1:length(noise_spike)
                noise_time = locs(noise_spike(noise));
                ephys.spike_binary(channel_interest,stim_num,noise_time-450:noise_time+450) = 0; % within 0.01 sec all the artifects will not be counted
            end
        end
    end
...
dlgTitle = 'User-defined input';
nLines = 1;
def = {'2','7'};% For spontaneous one use 1 and 2
answer = inputdlg(prompt, dlgTitle, nLines, def);
...
for stim_num = 1:length(PulseSeg) % should be 36 stimuli
    ...
    % the default is 1 seconds after the stimulus is off, 30000 samples = 1 sec

    ephys.no_stimulus_stop(stim_num) = str2num(answer{2})*30000+0.5*30000+30000;
    % the default is 3 seconds after the stimulus is off, 30000 samples = 1 sec
...
for channel_interest = 1:length(ephys.channel)
    ...
    ephys.spikeRateMean_2ms10Hz_nostim(channel_interest) = nanmean(ephys.spike_nostim_epoch(channel_interest,spike_locs))./no_stimulus_window;
end
...
for channel_interest = 1:length(ephys.channel)
    ...
    ephys.spikeRateMean_2ms20Hz_nostim(channel_interest) = nanmean(ephys.spike_nostim_epoch(channel_interest,spike_locs))./no_stimulus_window;
end
...
for channel_interest = 1:length(ephys.channel)
    ...
    ephys.spikeRateMean_2ms40Hz_nostim(channel_interest) = nanmean(ephys.spike_nostim_epoch(channel_interest,spike_locs))./no_stimulus_window;
end
...
for channel_interest = 1:length(ephys.channel)
    ...
    ephys.spikeRateMean_5ms10Hz_nostim(channel_interest) = nanmean(ephys.spike_nostim_epoch(channel_interest,spike_locs))./no_stimulus_window;
end
...
for channel_interest = 1:length(ephys.channel)
    ...
    ephys.spikeRateMean_5ms20Hz_nostim(channel_interest) = nanmean(ephys.spike_nostim_epoch(channel_interest,spike_locs))./no_stimulus_window;
end
...
for channel_interest = 1:length(ephys.channel)
    ...
    ephys.spikeRateMean_5ms40Hz_nostim(channel_interest) = nanmean(ephys.spike_nostim_epoch(channel_interest,spike_locs))./no_stimulus_window;
end
...
for channel_interest = 1:length(ephys.channel)
    ...
    ephys.spikeRateMean_10ms10Hz_nostim(channel_interest) = nanmean(ephys.spike_nostim_epoch(channel_interest,spike_locs))./no_stimulus_window;
end
...
for channel_interest = 1:length(ephys.channel)
    ...
    ephys.spikeRateMean_10ms20Hz_nostim(channel_interest) = nanmean(ephys.spike_nostim_epoch(channel_interest,spike_locs))./no_stimulus_window;
end
...
for channel_interest = 1:length(ephys.channel)
    ...
    ephys.spikeRateMean_10ms40Hz_nostim(channel_interest) = nanmean(ephys.spike_nostim_epoch(channel_interest,spike_locs))./no_stimulus_window;
end
...
for channel_interest = 1:length(ephys.channel)
    ...
    ephys.spikeRateMean_20ms10Hz_nostim(channel_interest) = nanmean(ephys.spike_nostim_epoch(channel_interest,spike_locs))./no_stimulus_window;
end
...
for channel_interest = 1:length(ephys.channel)
    ...
    ephys.spikeRateMean_20ms20Hz_nostim(channel_interest) = nanmean(ephys.spike_nostim_epoch(channel_interest,spike_locs))./no_stimulus_window;
end
...
for channel_interest = 1:length(ephys.channel)
    ...
    ephys.spikeRateMean_20ms40Hz_nostim(channel_interest) = nanmean(ephys.spike_nostim_epoch(channel_interest,spike_locs))./no_stimulus_window;
end
...
subplot(3,4,1)
x = [1 2];
y = [mean(ephys.spikeRateMean_2ms10Hz_stim) mean(ephys.spikeRateMean_2ms10Hz_nostim)];
...
data2 = ephys.spikeRateMean_2ms10Hz_nostim;
...
subplot(3,4,5)
x = [1 2];
y = [mean(ephys.spikeRateMean_2ms20Hz_stim) mean(ephys.spikeRateMean_2ms20Hz_nostim)];
value = bar(x,y,'FaceColor',[.7 .7 .7],'EdgeColor','none');
...
data2 = ephys.spikeRateMean_2ms20Hz_nostim;
...
subplot(3,4,9)
x = [1 2];
y = [mean(ephys.spikeRateMean_2ms40Hz_stim) mean(ephys.spikeRateMean_2ms40Hz_nostim)];
value = bar(x,y,'FaceColor',[.7 .7 .7],'EdgeColor','none');
...
data2 = ephys.spikeRateMean_2ms40Hz_nostim;
...
subplot(3,4,2)
x = [1 2];
y = [mean(ephys.spikeRateMean_5ms10Hz_stim) mean(ephys.spikeRateMean_5ms10Hz_nostim)];
value = bar(x,y,'FaceColor',[.7 .7 .7],'EdgeColor','none');
...
data2 = ephys.spikeRateMean_5ms10Hz_nostim;
...
subplot(3,4,6)
x = [1 2];
y = [mean(ephys.spikeRateMean_5ms20Hz_stim) mean(ephys.spikeRateMean_5ms20Hz_nostim)];
value = bar(x,y,'FaceColor',[.7 .7 .7],'EdgeColor','none');
...
data2 = ephys.spikeRateMean_5ms20Hz_nostim;
...
subplot(3,4,10)
x = [1 2];
y = [mean(ephys.spikeRateMean_5ms40Hz_stim) mean(ephys.spikeRateMean_5ms40Hz_nostim)];
value = bar(x,y,'FaceColor',[.7 .7 .7],'EdgeColor','none');
...
data2 = ephys.spikeRateMean_5ms40Hz_nostim;
...
subplot(3,4,3)
x = [1 2];
y = [mean(ephys.spikeRateMean_10ms10Hz_stim) mean(ephys.spikeRateMean_10ms10Hz_nostim)];
value = bar(x,y,'FaceColor',[.7 .7 .7],'EdgeColor','none');
...
data2 = ephys.spikeRateMean_10ms10Hz_nostim;
...
subplot(3,4,7)
x = [1 2];
y = [mean(ephys.spikeRateMean_10ms20Hz_stim) mean(ephys.spikeRateMean_10ms20Hz_nostim)];
value = bar(x,y,'FaceColor',[.7 .7 .7],'EdgeColor','none');
...
data2 = ephys.spikeRateMean_10ms20Hz_nostim;
...
subplot(3,4,11)
x = [1 2];
y = [mean(ephys.spikeRateMean_10ms40Hz_stim) mean(ephys.spikeRateMean_10ms40Hz_nostim)];
value = bar(x,y,'FaceColor',[.7 .7 .7],'EdgeColor','none');
...
data2 = ephys.spikeRateMean_10ms40Hz_nostim;
...
subplot(3,4,4)
x = [1 2];
y = [mean(ephys.spikeRateMean_20ms10Hz_stim) mean(ephys.spikeRateMean_20ms10Hz_nostim)];
value = bar(x,y,'FaceColor',[.7 .7 .7],'EdgeColor','none');
...
data2 = ephys.spikeRateMean_20ms10Hz_nostim;
...
subplot(3,4,8)
x = [1 2];
y = [mean(ephys.spikeRateMean_20ms20Hz_stim) mean(ephys.spikeRateMean_20ms20Hz_nostim)];
value = bar(x,y,'FaceColor',[.7 .7 .7],'EdgeColor','none');
...
data2 = ephys.spikeRateMean_20ms20Hz_nostim;
...
subplot(3,4,12)
x = [1 2];
y = [mean(ephys.spikeRateMean_20ms40Hz_stim) mean(ephys.spikeRateMean_20ms40Hz_nostim)];
value = bar(x,y,'FaceColor',[.7 .7 .7],'EdgeColor','none');
...
data2 = ephys.spikeRateMean_20ms40Hz_nostim;
