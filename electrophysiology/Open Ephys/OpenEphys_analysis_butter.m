...
[fileName pathName]=uigetfile('*.mat','Select .mat file',...
    'D:\Data\optogenetics\Open Ephys\analized data\20211027');
...
% Using butter filter for spikes
for channel_interest = 1:length(ephys.channel)
    ephys.raw(channel_interest,:) = voltage(channel_interest,:);
    [b,a] = butter(4, [0.016 0.407]); % Wn = Fc/(Fs/2), Fs = 30kHz, bandpass range: 244 Hz and 6.104 kHz
    ephys.Spike_filtered(channel_interest,:) = filtfilt(b,a,double(voltage(channel_interest,:)));
end
